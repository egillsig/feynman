...
(deftest substitutions
  ...
  (testing "apply-subs"
    ...
    (let [s {'d1 {}
             ...
             'd3 {"M" 1}
             'd4 {'d3 2}}]
      (is (= (apply-subs s {'d1 4})
             {}))
      (is (= (apply-subs s {'d2 1})
             {"L" 1 "T" -2}))
      (is (= (apply-subs s {'d2 2})
             {"L" 2 "T" -4}))
      (is (= (apply-subs s {'d2 1 'd3 -1})
             {"L" 1 "T" -2 "M" -1}))
      (is (= (apply-subs s {'d4 2})
             {"M" 4}))
      (is (= (apply-subs s {'d5 1})
             {'d5 1})))))
...
(deftest unification
  (testing "dim-unify-one"
    ...
    (let [d {'d__1 2
      ...
      (is (= {} (apply-subs (unify-with-one d) d))))
